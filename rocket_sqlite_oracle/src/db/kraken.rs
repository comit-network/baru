...
mod connection {
    ...
    pub async fn new(ws_url: Url) -> Result<BoxStream<'static, Result<wire::PriceUpdate, Error>>> {
        ...
        rate_stream
            .send(SUBSCRIBE_BTC_USD_TICKER_PAYLOAD.into())
            .await?;
    ...
    const SUBSCRIBE_BTC_USD_TICKER_PAYLOAD: &str = r#"
    { "event": "subscribe",
      "pair": [ "XBT/USD" ],
      "subscription": {
        "name": "ticker"
...
mod wire {
    ...
    pub enum Error {
        ...
        #[error("Bid Rate Element is of unexpected type")]
        UnexpectedBidRateElementType,
        #[error("Bid Rate Element is missing")]
        MissingBidRateElementType,
    ...
    impl TryFrom<TickerUpdate> for PriceUpdate {
        ...
        fn try_from(value: TickerUpdate) -> Result<Self, Error> {
            ...
            let ask = match ask {
                RateElement::Text(ask) => USDPrice::from_str(ask)?,
                _ => return Err(Error::UnexpectedAskRateElementType),
            };

            let bid = data.bid.first().ok_or(Error::MissingBidRateElementType)?;
            let bid = match bid {
                RateElement::Text(bid) => USDPrice::from_str(bid)?,
                _ => return Err(Error::UnexpectedBidRateElementType),
            };
    ...
    mod tests {
        ...
        fn can_deserialize_subscription_status_event() {
            let event = r#"{"channelID":980,"channelName":"ticker","event":"subscriptionStatus","pair":"XBT/USD","status":"subscribed","subscription":{"name":"ticker"}}"#;
        ...
        fn deserialize_ticker_update() {
            let message = r#"[980,{"a":["0.00440700",7,"7.35318535"],"b":["0.00440200",7,"7.57416678"],"c":["0.00440700","0.22579000"],"v":["273.75489000","4049.91233351"],"p":["0.00446205","0.00441699"],"t":[123,1310],"l":["0.00439400","0.00429900"],"h":["0.00450000","0.00450000"],"o":["0.00449100","0.00433700"]},"ticker","XBT/USD"]"#;

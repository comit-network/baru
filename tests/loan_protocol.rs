...
use elements::{
    opcodes, Address, AddressParams, AssetId, OutPoint, Script, SigHashType, Transaction, TxOut,
    Txid,
};
...
fn derive_blinding_key(master_blinding_key: Vec<u8>, script_pubkey: Script) -> Result<SecretKey> {
    use hmac::{Hmac, Mac, NewMac};
    use sha2::Sha256;

    let mut mac =
        Hmac::<Sha256>::new_varkey(&master_blinding_key).expect("HMAC can take key of any size");
...
impl Wallet {
    ...
    async fn find_inputs(&self, asset: AssetId, target_amount: Amount) -> Result<Vec<Input>> {
        ...
        let selected_coins = utxos
            ...
            .find_map(|(outpoint, amount, tx_out, _)| {
                (amount >= target_amount.as_sat()).then(|| Input {
                    txin: outpoint,
